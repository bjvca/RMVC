...
saveRDS(balance, file= paste(path,"mock_report/balance.RData", sep="/"))
...
for (i in 1:length(outcomes)) {
  ...
  vcov_cluster <- vcovCR(ols, cluster = farmers$catchment_ID, type = "CR0")
  res <- coef_test(ols, vcov_cluster)
  conf <- conf_int(ols, vcov_cluster)
...
mod <- summary(lm((lactoscan=="T") ~hh_size+age_head+herd_size+improved_share+liter_day_wet+liter_sold_day_wet+sell_MCC_wet+use_steel+coop_member+acaracide_exp, data=farmers))
F_test[2,2] <- pf(mod$fstatistic[1],mod$fstatistic[2],mod$fstatistic[3],lower.tail=FALSE)
F_test[1,2] <- mod$fstatistic[1]

summary(lm(video_shown ~hh_size+age_head+herd_size+improved_share+liter_day_wet+liter_sold_day_wet+sell_MCC_wet+use_steel+coop_member+acaracide_exp, data=farmers))
summary(lm((video_shown*(lactoscan=="T")) ~hh_size+age_head+herd_size+improved_share+liter_day_wet+liter_sold_day_wet+sell_MCC_wet+use_steel+coop_member+acaracide_exp, data=farmers))
F_test[1]
