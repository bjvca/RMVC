...
## 02_analysis_main.R
##
## Experiment 1 regressions: 2x2 factorial RCT (Lactoscan x Video)
## Ported from audited PAP/analysis/analysis.R and balance_tables.R
##
## Must be run with working directory set to paper/analysis/.
## Sources 00_utilities.R for helper functions (ihs, run_farmer_regressions, etc.)
##
## Outputs (saved to paper/results/):
##   balance.rds               — MCC-level balance array (10 x 8)
##   balance_farmer.rds        — farmer-level balance array (10 x 32)
##   F_test.rds                — joint F-test array (2 x 8)
##   res_farmers.rds           — primary farmer ITT (5 x 33)
##   res_farmers_TOT.rds       — primary farmer LATE (5 x 33)
##   res_farmers_sec_quant.rds — secondary: sales quantities (5 x 32)
##   res_farmers_sec_uptake.rds— secondary: treatment uptake (5 x 33)
##   res_farmers_sec_sold.rds  — secondary: sales/market channel (6 x 32)
##   res_farmers_sec_switching.rds — secondary: switching (4 x 32)
...
## ---- A3. Farmer-level balance (10 outcomes x 32 columns) ------------------
##
## Column layout (mirrors res_farmers):
##  1-2:   ctrl_mean, ctrl_sd  (pure control: lactoscan=C & video=FALSE)
##  3-5:   T1 (lactoscan) coef, se, p     [fully interacted model]
##  6-8:   T2 (video) coef, se, p         [fully interacted model]
##  9-12:  T1xT2 interaction coef, se, p, nobs
##  13-15: het p-values: T1, T2, T1xT2 effects differ for traders
...
##  27-29: bundle coef, se, p  [pure control vs full bundle]
##  30:    bundle x trader interaction p-value
##  31:    Anderson-Sharp q-value for col 29
##  32:    (placeholder for layout consistency)
...
farmers_bal_int <- farmers_balance[
  (farmers_balance$video_shown == TRUE  & farmers_balance$lactoscan == "T") |
  (farmers_balance$video_shown == FALSE & farmers_balance$lactoscan == "C"), ]
...
for (i in seq_along(outcomes_farmer)) {
  ...
  balance_farmer[i, 9:12] <- c(res[5, 2], res[5, 3], res[5, 7],   # T1xT2
                                nobs(ols))

  ## Heterogeneity tests: does treatment effect differ for traders?
  ...
  ## --- Bundle comparison: pure control vs full bundle ---
  ols_int <- lm(as.formula(paste(outcomes_farmer[i], "~ lactoscan * trader")),
                data = farmers_bal_int)
  vcov_int <- vcovCR(ols_int, cluster = farmers_bal_int$catchment_ID, type = "CR2")
  res_int  <- coef_test(ols_int, vcov_int)

  balance_farmer[i, 27:29] <- c(res_int[2, 2], res_int[2, 3], res_int[2, 7])
  balance_farmer[i, 30]    <- res_int[4, 7]  # lactoscanT:trader interaction
}

## Anderson-Sharp q-values for fully interacted model p-values
balance_farmer[, 16] <- anderson_sharp_q(balance_farmer[, 5])    # T1
balance_farmer[, 17] <- anderson_sharp_q(balance_farmer[, 8])    # T2
balance_farmer[, 18] <- anderson_sharp_q(balance_farmer[, 11])   # T1xT2
balance_farmer[, 31] <- anderson_sharp_q(balance_farmer[, 29])   # bundle
...
##  3-5:   treat coef, SE, p  (full model)
##  6-8:   vid coef, SE, p    (full model)
##  9-11:  treat:vid coef, SE, p (full model)
##  12:    N observations
##  13:    het p: treat differs by trader
##  14:    het p: vid differs by trader
##  16-18: q-values for cols 5, 8, 11
##  19-21: orthog treat coef, SE, p (demeaned vid)
##  22-24: orthog vid coef, SE, p (demeaned treat)
##  25-26: q-values for cols 21, 24
##  27-29: bundle coef, SE, p
##  33:    het p: treat:vid differs by trader
...
## ---------------------------------------------------------------------------
## B1. Fully interacted ITT: outcome ~ treat*vid*trader + b_outcome
## ---------------------------------------------------------------------------
## With baseline control, coefficient ordering:
...
for (i in seq_along(outcomes)) {
  ...
  res_farmers[i, 9:12]  <- c(res[6, 2], res[6, 3], res[6, 7],   # treat:vid
                              nobs(ols))

  ## Heterogeneity tests: does effect differ by trader?
  res_farmers[i, 13] <- linearHypothesis(
    ols, "treatTRUE = treatTRUE:trader", vcov. = vcov_cluster)[[4]][2]
  res_farmers[i, 14] <- linearHypothesis(
    ols, "vidTRUE = vidTRUE:trader", vcov. = vcov_cluster)[[4]][2]
  res_farmers[i, 33] <- linearHypothesis(
...
###############################################################################
## Same outcome family as ITT, but instrumenting actual machine use
## (treat_TOT) with random assignment (treat).

res_farmers_TOT <- array(NA, dim = c(length(outcomes), 33))

## ---------------------------------------------------------------------------
## C1. Fully interacted 2SLS: outcome ~ treat_TOT*vid*trader + b_outcome
##     Instruments: treat*vid*trader + b_outcome
## ---------------------------------------------------------------------------
for (i in seq_along(outcomes)) {
...
farmers_end$treat_TOT_demeaned <- farmers_end$treat_TOT -
  mean(farmers_end$treat_TOT, na.rm = TRUE)
...
###############################################################################
## Uses run_farmer_regressions() from 00_utilities.R

## Refresh bundle subset (may have changed after Anderson index addition)
farmers_end_int <- farmers_end[
  (farmers_end$vid == TRUE  & farmers_end$treat == TRUE) |
  (farmers_end$vid == FALSE & farmers_end$treat == FALSE), ]

## ---- D1. Sales quantities (has_baseline=TRUE, ncols=32) -------------------
...
res_farmers_sec_quant <- run_farmer_regressions(
  outcomes_sq, b_outcomes_sq, farmers_end, farmers_end_int,
...
## ---- D2. Treatment uptake (has_baseline=FALSE, ncols=33) ------------------
...
res_farmers_sec_uptake <- run_farmer_regressions(
  outcomes_up, b_outcomes = NULL, farmers_end, farmers_end_int,
...
## ---- D3. Sales/market channel (has_baseline=TRUE, ncols=32) ---------------
...
res_farmers_sec_sold <- run_farmer_regressions(
  outcomes_sold, b_outcomes_sold, farmers_end, farmers_end_int,
  has_baseline = TRUE, ncols = 32
)
...
## ---- D4. Switching (has_baseline=FALSE, ncols=32) -------------------------
...
res_farmers_sec_switching <- run_farmer_regressions(
  outcomes_sw, b_outcomes = NULL, farmers_end, farmers_end_int,
